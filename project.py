...
async def start_command(message: types.Message):
    ...
    await message.reply_text(f"Привет, {user_name}! Я бот, который может отправлять ссылки на разные ресурсы. Нажми на кнопку /help, чтобы узнать больше.", reply_markup=keyboard)
...
async def help_command(message: types.Message):
    ...
    await message.reply_text("Чем я могу тебе помочь? Нажми на одну из кнопок, чтобы перейти на Discord или на сайт Perm-ACOIN.", reply_markup=keyboard)
# Запускаем бота
